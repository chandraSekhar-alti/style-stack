...
public class RazorpayGateway implements PaymentGateway {
  ...
  // Gateway
  private PaymentLink createPaymentLinkFromGateway(JSONObject request) {
    ...
    try {
      ...
      throw new PaymentGatewayException(ex.getMessage(), ex);
      //            throw new PaymentGatewayException("Unable to generate Razorpay payment link.",
      // ex);
  ...
  private long getExpiryTime() {
    return Instant.now().plus(Duration.ofMinutes(30)).getEpochSecond();
  }
}
